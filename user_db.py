...
class User_db:
    ...
    # ====================================================================================================
    def sendtoUserFirebase(self,db, sensordata):
        ...
        for key in sensordata:
            ...
            result_t = db.child(userid).child(key).push(sensordata["Timestamp"], user["idToken"])  # send time of each record

        print(result, result_t)
        return


    # The function to set up the record structure to be written to the database.
    # ====================================================================================================
    def setupData(self, temp, humid, motion, smoke, rfid, timestamp):
    ...
    # ====================================================================================================
    def data_send(self, temp, humid, motion, smoke, rfid ):  # test code to send and receive data from the firebase database.
        print("start of the testFirebase")
        firebase = self.dbInitialization()
        if user is not None:  # if authorization is not failed.
            sensordata = self.setupData(temp, humid, motion, smoke, rfid,
            ...
            self.sendtoUserFirebase(firebase.database(), sensordata)  # save to the user specific userdata tree
